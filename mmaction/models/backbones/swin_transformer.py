...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if self.attn_type == 'normal':
            ...
            v = v.view((_B, _nH) + self.window_size + (_C, ))
            ...
            v = v.view(_B, _nH, new_N, _C)

            attn = q @ k.transpose(-2, -1)  # B_, nH, N, new_N

            relative_position_bias = self.relative_position_bias_table[
                self.relative_position_index[:N, W * H:N - W * H].reshape(
                    -1)].reshape(N, new_N, -1)  # Wd*Wh*Ww,(Wd-2)*Wh*Ww,nH
            relative_position_bias = relative_position_bias.permute(
                2, 0, 1).contiguous()  # nH, Wd*Wh*Ww, (Wd-2)*Wh*Ww
            ...
            if mask is not None:
                ...
                mask = mask[:, :, W * H:N - W * H]
                attn = attn.view(B_ // nW, nW, self.num_heads, N,
                                 new_N) + mask.unsqueeze(1).unsqueeze(0)
                attn = attn.view(-1, self.num_heads, N, new_N)
                attn = self.softmax(attn)
            else:
                attn = self.softmax(attn)
